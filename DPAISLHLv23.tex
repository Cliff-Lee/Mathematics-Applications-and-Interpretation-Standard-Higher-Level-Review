...
% --- Graphics / plotting (keep if you use them) ---
\usepackage{tikz}
...
\hypersetup{bookmarksopen=true,bookmarksnumbered=false,colorlinks=true,linkcolor=blue}
...
% --- Question/Solution environments + page-break rules ---

% (1) Counters
\newcounter{question}
\newcounter{solutionNo}      % assumes solutions appear in the same order as questions

% (2) Page-break behaviour:
%     - each \section (Topic) starts on a new page and resets the subpoint counter
%     - the 2nd and subsequent \subsection in a Topic also start on a new page
...
% (3) Environments
% --- Question/Solution environments with TOC backlink ---
% --- Question/Solution environments with TOC backlink, cleaner layout ---
\newenvironment{question}{
  \refstepcounter{question}%
  \hypertarget{q-\thequestion}{}%
  \par\noindent\textbf{Q\thequestion.}~%
  \hyperlink{sol-\thequestion}{\small[Go to Solution]}%
  \quad\hyperlink{toc}{\small[Back to TOC]}%
  \par\vspace{0.3em} % <<--- move text to next line with small gap
}{
  \par\vspace{1cm}%
  \noindent\textbf{Final Answer:}\ \hrulefill
  \vspace{1.5cm}
...
% Border that hugs between the header and footer areas
...
\AddEverypageHook{%
  \begin{tikzpicture}[remember picture,overlay]
    \draw[line width=0.9pt, rounded corners=2pt]
      % Top-left corner: just under header area, inset slightly to the left
      ($ (current page header area.south west) + (-\FramePadLR, -\FramePadTop) $)
        rectangle
      % Bottom-right corner: just above footer area, inset to the right
...
% Detailed Topic 1 questions from the original compilation
...
\tocsubsection{AHL  1.16  \; Adjacency  Matrices}
Use adjacency matrices to study graphs and count walks.

\begin{question}
The diagram shows a simple graph with vertices \(A,B,C\) and edges \(AB,BC,CA\).
In the vertex order \(A,B,C\), write the adjacency matrix of the graph.

\begin{center}
\begin{tikzpicture}[scale=1, node distance=2.2cm,
  every node/.style={circle, draw, minimum size=7mm, inner sep=0pt}]
  \node (A) at (0,0) {A};
  \node (B) at (2.2,0) {B};
  \node (C) at (1.1,1.9) {C};
  \draw (A)--(B)--(C)--(A); % complete graph on 3 vertices
\end{tikzpicture}
\end{center}
\end{question}
...
\begin{question}
Given the adjacency matrix $A=\begin{bmatrix}0&1&1\\1&0&1\\1&1&0\end{bmatrix}$, compute $A^2$ and interpret the entry $(1,1)$.
\end{question}

%------------------------------------------------------------------------------
\tocsubsection{Topic 2 — Functions (SL 2.1–2.6, AHL 2.7–2.10)}
% The complete set of Topic 2 questions from the original combined document.
% These exercises cover straight lines, relations and functions, families of
% functions, composite and inverse functions, transformations of graphs and
% additional modelling families.  Each question has been reformulated using
% the unified \texttt{question} environment, which automatically leaves space
...
\tocsubsection{AHL 4.18 \; Markov Chains}
...
% AHL 5.17 — Phase portraits for linear planar systems
% (dx/dt = ax + by, dy/dt = cx + dy)
% NOTE: Only require exact solutions when eigenvalues are real, distinct (per syllabus).
% Otherwise, ask for qualitative classification and sketches.
...
% ---------------- AHL 5.18 — Questions ----------------
% Solutions of d^2x/dt^2 = f(x, dx/dt, t) by Euler’s method and exact solutions (as AHL 5.17).
% Use coupled first-order form:  x' = y,  y' = f(x,y,t).
% Unless stated otherwise, take g=9.8 and all constants positive.
...
\begin{solution}
The adjacency matrix of $K_3$ in order $A,B,C$ is
...
\end{bmatrix}.
\]
The entry \((1,1) = 2\) counts the number of distinct walks of length \(2\) starting and ending at vertex \(A\).
Here, there are two such walks: \(A \to B \to A\) and \(A \to C \to A\).
\end{solution}
...
\tocsubsection{AHL 4. \; Hypothesis testing and confidence intervals}

\begin{solution}
\textbf{Hypothesis test (one-sample $z$).}  Known $\sigma=8$, $n=30$, $\bar x=52$.
\[
H_0:\mu=50,\qquad H_1:\mu>50 \quad (\text{right–tailed}).
\]
Test statistic:
\[
z=\frac{\bar x-\mu_0}{\sigma/\sqrt{n}}
=\frac{52-50}{8/\sqrt{30}}
=\frac{2\sqrt{30}}{8}
=\frac{\sqrt{30}}{4}\approx 1.37.
\]
$p$–value $=1-\Phi(1.37)\approx 0.085$.  
At the $5\%$ level, the critical value is $z_{0.95}=1.645$; since $1.37<1.645$ (equivalently $p=0.085>0.05$), \textbf{fail to reject $H_0$}.  

\textbf{Conclusion:} There is not sufficient evidence at the $5\%$ level to conclude that the mean lifetime exceeds $50$ hours.

\textbf{Final Answer:} $H_0:\mu=50$, $H_1:\mu>50$; $z\approx1.37$; $p\approx0.085$; fail to reject $H_0$.
\end{solution}
